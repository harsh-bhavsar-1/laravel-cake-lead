...
trait Leadscore2 {
    ...
    public function validatePhone2($phoneNumber,$first_name,$last_name,$email,$state,$zipcode,$fullresponse) {

        $response = $this->makeRequest2($phoneNumber,$first_name,$last_name,$email,$state,$zipcode);

        if ($response) {
        	if ($fullresponse === false) {
            	$result = $this->decodeResponse2($response);
                return $this->testResult2($result);
            } else {
            	$result = $this->decodeResponse2($response);
    ...
    private function decodeResponse2($response) {
    ...
    private function testResult2($result) {
    ...
    private function makeRequest2($phoneNumber,$first_name,$last_name,$email,$state,$zipcode) {
